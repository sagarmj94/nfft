...
const [istxLoad, setIstTxLoad] = useState(false)
...
const getTransection = () => {
	...
	axios(config)
		.then(function (response) {
			...
			setTxnLoading(false)
			setIstTxLoad(false)
			console.log("txn true")
		})
		.catch(function (error) {
			console.log("txn catch")
			setTxnLoading(true)
			setIstTxLoad(false)
														...
														?.map((item, id) => {
															return (
																<tr key={id}>
																	...
																	<td>{parseFloat(item?.owned_asset_count)}</td>
																</tr>
															)
														})
												)}
														...
														return (
															<tr key={id}>
																...
																<td className='font-weight-600'>
																	{`${getPrice(topTxn, wallletData?.address)} ${
																...
																<td>
																	<div>
																		...
																		{/* <span className=''>Thomas Cook</span> */}
																		<span className=''>
																			{getToValue(topTxn, wallletData?.address)}
																		</span>
																	</div>
																</td>
